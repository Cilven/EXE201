...
class Productcontrollers {
  ...
  ShowDetailCombo = (req, res, next) => {
    Combo.findOne({ slug: req.params.slug })
      .populate("products.productId") // Lấy thông tin chi tiết sản phẩm trong combo
      .lean()
      .then((combo) => {
        if (!combo) {
          return res.status(404).send("Combo không tồn tại!");
        }
        console.log("Combo data:", combo); // Kiểm tra dữ liệu lấy từ DB
        res.render("product/sshowcombo", { combo });
      })
      .catch((error) => {
